/*
 * sha/sha512_local.h: low-level implementation of SHA512 algorithm
 *
 * ====================================================================
...
void xdig_sha512_ctx_final(xdig_sha512_ctx_t *c, unsigned char *md)
{
...
#ifdef  B_ENDIAN
    c->u.d[XDIG_SHA_LBLOCK - 2] = c->Nh;
    c->u.d[XDIG_SHA_LBLOCK - 1] = c->Nl;
#else
    ...
    case XDIG_SHA224_DIGEST_LENGTH:
        for (n = 0; n < XDIG_SHA224_DIGEST_LENGTH / 8; n++) {
            XDIG_SHA_LONG64 t = c->h[n];
         ...
         */
        {
            XDIG_SHA_LONG64 t = c->h[XDIG_SHA224_DIGEST_LENGTH / 8];
        ...
        break;
    case XDIG_SHA256_DIGEST_LENGTH:
        for (n = 0; n < XDIG_SHA256_DIGEST_LENGTH / 8; n++) {
            XDIG_SHA_LONG64 t = c->h[n];
        ...
        break;
    case XDIG_SHA384_DIGEST_LENGTH:
        for (n = 0; n < XDIG_SHA384_DIGEST_LENGTH / 8; n++) {
            XDIG_SHA_LONG64 t = c->h[n];
        ...
        break;
    case XDIG_SHA512_DIGEST_LENGTH:
        for (n = 0; n < XDIG_SHA512_DIGEST_LENGTH / 8; n++) {
            XDIG_SHA_LONG64 t = c->h[n];
...
#    define ROTR(a,n)    ({ XDIG_SHA_LONG64 ret;             \
                                asm ("rorq %1,%0"       \
                                : "=r"(ret)             \
                                : "J"(n),"0"(a)         \
                                : "cc"); ret;           })
#    if !defined(B_ENDIAN)
#     define PULL64(x) ({ XDIG_SHA_LONG64 ret=*((const XDIG_SHA_LONG64 *)(&(x)));  \
                                asm ("bswapq    %0"             \
                                : "=r"(ret)                     \
                                : "0"(ret)); ret;               })
#    endif
...
#     define PULL64(x)   ({ XDIG_SHA_LONG64 ret;                     \
                                asm ("rev       %0,%1"          \
                                : "=r"(ret)                     \
                                : "r"(*((const XDIG_SHA_LONG64 *)(&(x))))); ret; })
#    endif
...
#    define Sigma0(x) ({ XDIG_SHA_LONG64 ret; unsigned int *r = (unsigned int *)(&(ret));    \
                        const unsigned int *p = (const unsigned int *)(&(x));           \
                        ...
                        : "=r"(r[1])                                                    \
                        : "r" (p[0]), "r" (p[1])); ret;                                 })
#    define Sigma1(x) ({ XDIG_SHA_LONG64 ret; unsigned int *r = (unsigned int *)(&(ret));    \
                        const unsigned int *p = (const unsigned int *)(&(x));           \
...
#    define Sigma0(x) ({ XDIG_SHA_LONG64 ret;            \
                        asm ("sha512sum0 %0, %1"    \
                        : "=r"(ret)                 \
                        : "r"(x)); ret;             })
#    define Sigma1(x) ({ XDIG_SHA_LONG64 ret;            \
                        asm ("sha512sum1 %0, %1"    \
                        : "=r"(ret)                 \
                        : "r"(x)); ret;             })
#    define sigma0(x) ({ XDIG_SHA_LONG64 ret;            \
...
#    define Ch(x,y,z) ({  XDIG_SHA_LONG64 ret; unsigned int *r = (unsigned int *)(&(ret));   \
...
#    define Maj(x,y,z) ({ XDIG_SHA_LONG64 ret; unsigned int *r = (unsigned int *)(&(ret));   \
                        const unsigned int *xp = (const unsigned int *)(&(x));          \
                        const unsigned int *yp = (const unsigned int *)(&(y));          \
                        const unsigned int *zp = (const unsigned int *)(&(z));          \
                        asm (".insn r4 0x33, 1, 0x3, %0, %2, %1, %3\n\t"                \
...
#    define Ch(x,y,z) ({  XDIG_SHA_LONG64 ret;                           \
                        asm (".insn r4 0x33, 1, 0x3, %0, %2, %1, %3"\
                        : "=r"(ret)                                 \
                        : "r"(x), "r"(y), "r"(z)); ret;             })
#    define Maj(x,y,z) ({ XDIG_SHA_LONG64 ret;                           \
                        asm (".insn r4 0x33, 1, 0x3, %0, %2, %1, %3"\
                        : "=r"(ret)                                 \
                        : "r"(x^z), "r"(y), "r"(x)); ret;           })
#   endif
...
#   endif
#   if defined(_M_IX86) && !defined(xdig_NO_ASM) && \
       !defined(xdig_NO_INLINE_ASM)
#    if defined(I386_ONLY)
static XDIG_SHA_LONG64 __fastcall __pull64be(const void *x)
{
    _asm mov  edx,[ecx + 0]
    _asm mov  eax,[ecx + 4]
    _asm xchg dh, dl
...
# endif
# ifndef PULL64
#  define B(x,j)    (((XDIG_SHA_LONG64)(*(((const unsigned char *)(&x))+j)))<<((7-j)*8))
